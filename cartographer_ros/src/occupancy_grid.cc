...
void BuildOccupancyGrid(
  ...
  for (const Eigen::Array2i& xy_index :
    ...
    if (probability_grid.IsKnown(xy_index + offset)) {
      int value = carto::common::RoundToInt(
...
void BuildOccupancySubGrid(
  ...
  const::cartographer::mapping::TrajectoryNode  &current_node=trajectory_nodes.back();
  ...
  carto::mapping_2d::LaserFanInserter laser_fan_inserter(
    ...
    CHECK(current_node.constant_data->laser_fan_3d.returns.empty());  // No 3D yet.
    laser_fan_inserter.Insert(
        carto::sensor::TransformLaserFan(
            current_node.constant_data->laser_fan,
            carto::transform::Project2D(current_node.pose).cast<float>()),
        &probability_grid);

  occupancy_grid->header.stamp = ToRos(current_node.time());
  occupancy_grid->header.frame_id = options.map_frame;
  occupancy_grid->info.map_load_time = occupancy_grid->header.stamp;
